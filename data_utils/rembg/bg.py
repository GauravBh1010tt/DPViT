...
img_dir="/home/gaurav/borg/workspace/concept_learning/data/tieredimagenet/train/"
...
for fold in folders[:1]:
    #out_path = out_dir+'/'+fold.split('/')[-1]
    #os.mkdir(out_path)
    files = glob.glob(fold+"/*")
    data = []
    for im_path in tqdm(files[1:12]):
        img = Image.open(im_path)
        data.append(np.array(img))

    a = remove_many(data,net)
    print (a.shape)
    bre