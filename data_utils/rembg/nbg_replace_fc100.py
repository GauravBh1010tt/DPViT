...
img_dir = '/ubc/cs/research/shield/datasets/ILSVRC2012/train/'
save_root = "/ubc/cs/home/g/gbhatt/borg/concept_learning/data/Imnet/train/" # miniIM, tIM, cub 

batch = 30

#if dataset == 'mini':
#img_dir="../"+dataset+"/train/"
#img_dir="../"+dataset+"/n00000062/"

folders = glob.glob(img_dir+"*")

for fold in folders[500:750]:
    
    files = glob.glob(fold+"/*")
    #files = folders
    #print (fold)
    print (fold, len(files))
    save_path = save_root+fold.split('/')[-1]
    ...
    for idx in tqdm(range(0, len(files), batch)):
        ...
        for im_path in files[idx:idx+batch]:
            img = Image.open(im_path).convert('RGB').resize((480,480))
            ...
            save_paths.append(save_path+'/'+im_path.split('/')[-1])
