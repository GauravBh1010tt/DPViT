...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_wo_soft = False, args=None):
        super().__init__()
    ...
    def forward(self, x, return_attention=False, curr_layer_mask = None):
        ...
        y, attn = self.s_attn(self.norm1(x), curr_layer_mask)
        ...
        if self.args.use_parts:
            ...
            dmap, z = torch.zeros(x.shape[0], attn.shape[-1], self.args.K).cuda(), torch.zeros_like(y)
            c_attn = attn
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, is_student=False, frozen_patch_embed = False, 
                 args=None, attn_wo_soft = False, **kwargs):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                attn_wo_soft = attn_wo_soft, args=args)
    ...
    def compute_mix(self, dmap, eval_mode=False):
        ...
        # if self.args.l_noise and not eval_mode:
        #     fore_code = fore_code + self.args.l_noise*G_noise
        #     back_code = back_code + self.args.l_noise*G_noise
        ...
        if eval_mode:
            act_parts = F.interpolate(out, size=(resize,resize), mode='bilinear', align_corners=False)
            return fore_mix, back_mix, act_parts
        else:
