...
@torch.no_grad()
def main_eval(rank=None, world_size=None, args=None):
    ...
    valdir = os.path.join(args.data_path, args.partition)
    ...
    ce_loss = loss_functions.CELoss(nclasses = args.num_classes,
                    in_dim = args.out_dim, batch_size = args.batch_size_per_gpu ).cuda()
    
    if args.n_gpus>1:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[rank], output_device=rank, 
                                                          find_unused_parameters=False)
    #args = utils.restart_from_checkpoint(args.pretrained_weights, args=args, ce_loss=ce_loss, **{args.checkpoint_key: model})
    args = utils.restart_from_checkpoint(args.pretrained_weights, args=args, teacher=model, ce_loss=ce_loss)
    model.eval()

    # ============ evaluate unsup cls ... ============
    print("Evaluating unsupervised classification for val set...")
    eval_sup(model, data_loader_val, ce_loss, args)

@torch.no_grad()
def eval_sup(model, data_loader, ce_loss, args=None):
    metric_logger = utils.MetricLogger(delimiter="  ")
    real_labels, pred_labels = [], []
    for samples, labels in metric_logger.log_every(data_loader, 100):
        samples = samples.cuda(non_blocking=True)
        labels = labels.cuda(non_blocking=True)

        output = model(samples)
        if args.use_DDP:
            pred = utils.concat_all_gather(ce_loss.linear(output).max(dim=1)[1],args=args)
            real_labels.append(utils.concat_all_gather(labels,args=args))
        else:
            pred = ce_loss.linear(output).max(dim=1)[1]
    ...
    acc = metrics.accuracy_score(pred_labels, real_labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_gpus', default=1, type=int, help='Patch resolution of the model.')
    ...
    parser.add_argument('--sdc_topk', default=5, type=int)
    parser.add_argument('--decay_mix', default=1, type=float)
    parser.add_argument('--masked_fore', default=0, type=float)
    parser.add_argument('--lambda_nc', default=1, type=float)
    parser.add_argument('--lambda_sdc', default=1, type=float)
    parser.add_argument('--lambda_inv_ft', default=1, type=float)
    parser.add_argument('--lambda_inv_p', default=1, type=float)
    parser.add_argument("--mix_loss", type=str, default='L2', choices=["dice","L1","KL","L2"])
    ...
    for checkpoint_key in args.checkpoint_key.split(','):
        ...
        if args.n_gpus>1:
            mp.spawn(
                    main_eval,
                    args=(args_copy.n_gpus, args_copy, ),
                    nprocs=args.n_gpus,
                    daemon=False,
                    join=True,
                )
            exit()
        main_eval(args=args_copy)
