...
def restart_from_checkpoint(ckp_path, run_variables=None, args=None, preempt_flag=False, **kwargs):
    """
...
def dist_setup(args):
    ...
    dist.init_process_group(backend=args.dist_backend, init_method=args.init_method, world_size=args.n_gpus, rank=rank)
    print("process group ready!")

    return current_device

def local_setup(rank, world_size, port='9987'):
...
class MultiCropWrapper_dpvit(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    def __init__(self, backbone, head=None):
        super(MultiCropWrapper_dpvit, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        if head is None:
    ...
    def forward(self, x, mask=None, return_backbone_feat=False, \
                return_attention=False, return_dmaps=False, **kwargs):
        # convert to list
        if not isinstance(x, list):
            x = [x]
        ...
        for end_idx in idx_crops:
            ...
            _out, attn, mix = self.backbone(inp_x, return_attention=return_attention, **kwargs)
            if start_idx == 0:
                output = _out
            else:
                output = torch.cat((output, _out))
        ...
        if return_backbone_feat:
            return output, output_, attn, mix
        else:
            return output_, attn, mix


class DataAugmentation_DPViT(object):
    def __init__(self, global_crops_scale, local_crops_scale, global_crops_number, local_crops_number, 
                 global_crops_size, local_crops_size, dataset_name, args=None):
        ...
        if 'mini' in dataset_name or 'tiered' in dataset_name or 'in9' in dataset_name or 'Imagenet' in dataset_name:
        ...
        self.org_transfo_mask = transforms.Compose([
            transforms.ToTensor(),
        ])
        ...
        self.corruptions = transforms.Compose([
            #transforms.RandomResizedCrop(global_crops_size, scale=global_crops_scale, interpolation=Image.BICUBIC),
            color_jitter,
            GaussianBlur(0.1),
            Solarization(0.2),
    ...
    def __call__(self, img):
        ...
        if self.args.use_trC:
            if 'mini' in self.args.dataset_name or 'tiered' in self.args.dataset_name or 'in9' in self.args.dataset_name or 'Imagenet' in self.args.dataset_name:
                img_left_area = (0, 0, 480, 480)
                img_right_area = (480, 0, 480*2, 480)
                #print ('here')
            else:
                img_left_area = (0, 0, 84, 84)
                img_right_area = (84, 0, 84*2, 84)

            image = img.crop(img_left_area)
            fore_im = img.crop(img_right_area)

        rnd_flip1, rnd_flip2 = random.random()>0.5, random.random()>0.5

        rrc = transforms.RandomResizedCrop(self.args.global_crops_size, scale=self.args.global_crops_scale, 
                                            interpolation=Image.BICUBIC)

        i, j, h, w = rrc.get_params(img=image, ratio=rrc.ratio, scale=rrc.scale)

        im = TF.resized_crop(image, i, j, h, w, rrc.size, rrc.interpolation)
        fore = TF.resized_crop(fore_im, i, j, h, w, rrc.size, rrc.interpolation)

        if rnd_flip1:
            crops.append(self.global_transfo1(TF.hflip(im)))
            fore_imgs.append(transforms.ToTensor()(TF.hflip(fore)))
        else:
            crops.append(self.global_transfo1(im))
            fore_imgs.append(transforms.ToTensor()(fore))
        ...
        if rnd_flip2:
            crops.append(self.global_transfo2(TF.hflip(im)))
            fore_imgs.append(transforms.ToTensor()(TF.hflip(fore)))
        else:
            crops.append(self.global_transfo2(im))
            fore_imgs.append(transforms.ToTensor()(fore))

        for _ in range(self.local_crops_number):
            crops.append(self.local_transfo(image))

        return crops, fore_imgs
...
def evaluation(args, vits, epoch, rollout=False, foreground_threshold = 0.4, evaluation_method='cosine', top_k=5, 
    ...
    if args.arch in mymodels.__dict__.keys():
        model = mymodels.__dict__[args.arch](patch_size=args.patch_size, num_classes=0,args=args)
    else:
...
def evaluation_local(args, vits, epoch, rollout=False, foreground_threshold = 0.4):
...
def plot_attn(args, attentions, img, name, w_featmap, h_featmap):
    ...
    for j in range(nh):
        fname = os.path.join(args.visualize_output_dir, name+"-head" + str(j) + ".png")
    ...
    try:
        ...
        for idx in [(0,0), (0,1), (0,2), (1,0), (1,1), (1,2)]:
            ...
            i += 1
        #plt.show()
        fig.savefig(os.path.join(args.visualize_output_dir, name+"-heads" + ".png"), bbox_inches='tight', dpi = 500)
    except:
    ...
    ax.set_yticks([])
    #plt.show()
    fig.savefig(os.path.join(args.visualize_output_dir, name+"-combined" + ".png"), bbox_inches='tight', dpi = 500)
...
def gen_heat(up_sum):
    min_heat = torch.min(up_sum)
    heatmap11 = up_sum - min_heat
    heatmap_maxval = torch.max(torch.tensor(heatmap11))
    heatmap11 /= heatmap_maxval.item()
    heatmap1 = np.uint8(255 * heatmap11.numpy())
    heatmap = cv2.applyColorMap(heatmap1, cv2.COLORMAP_JET)

    return heatmap

def save_im(im, heat_mix_fore, heat_mix_back, heat_fore, heat_back, path='demo.jpg'):
    
    fig, ax = plt.subplots(3, 4, constrained_layout=True, figsize=(10, 10))

    ax[0,0].imshow(im.permute(1,2,0))
    ax[0,1].imshow(im.permute(1,2,0))
    ax[0,2].imshow(heat_mix_fore)
    ax[0,3].imshow(heat_mix_back)

    for i in range(len(heat_fore)):
        ax[1,i].imshow(heat_fore[i])
        ax[2,i].imshow(heat_back[i])
...
def visualize(args, vits, epoch, image_path = None, image_size = (480, 480), threshold = None, rollout=False):
    ...
    #args.visualize_output_dir = os.path.join(args.output_dir, "visualization_epoch{}".format(epoch), str(img_id))

    img_paths = glob.glob(os.path.join(args.image_path,'*'))
    # open image
    for img_id, im_path in enumerate(img_paths):
        ...
        out = model.get_last_selfattention(img.to(device), rollout)
        attentions, c_attns, dmap = out[1][0], out[1][1], out[2]

        plot_attn(args, attentions, img=img, name='attn', w_featmap=w_featmap, h_featmap=h_featmap)
        plot_attn(args, c_attns, img=img, name='c_attn', w_featmap=w_featmap, h_featmap=h_featmap)

        fore_mix, back_mix, act_parts = model.compute_mix(dmap, eval_mode=True)
        heat_mix_fore, heat_mix_back = gen_heat(fore_mix.detach().cpu().squeeze()), gen_heat(back_mix.detach().cpu().squeeze())

        #pdb.set_trace()

        heat_fore, heat_back = [], []
        for j in range(args.num_parts_viz):
            ind1,ind2 = np.random.randint(0,args.num_fore), np.random.randint(args.num_fore,args.K)

            heat_fore.append(gen_heat(act_parts[0][ind1].cpu().detach()))
            heat_back.append(gen_heat(act_parts[0][ind2].cpu().detach()))

        #pdb.set_trace()
        save_im(img[0], heat_mix_fore, heat_mix_back, heat_fore, heat_back, args.visualize_output_dir)
...
def concat_all_gather(tensor, args=None):
    """
