...
def mix_loss(inputs, targets, smooth=1, loss_type='dice', global_views=1, args=None):
	#comment out if your model contains a sigmoid or equivalent activation layer

    # pdb.set_trace()
    # inputs = inputs[0:global_views*]
    ...
    if loss_type == 'dice':
        ...
        loss = (2.*intersection + smooth)/(inputs.sum() + targets.sum() + smooth)
    elif loss_type == 'L1':
...
#def part_contraints(fore_code, back_code, dmaps):
...
class IBOT_Loss(nn.Module):
    def __init__(self, out_dim, patch_out_dim, ngcrops, nlcrops, warmup_teacher_temp, 
                 ...
                 batch_size_per_gpu = None, patch_num_global_crops = None, weighted_pool=False, args=None
                 ):
        super().__init__()
        self.args = args
        self.student_temp = student_temp
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, epoch, labels=None, teacher_backbone=None):
        ...
        if type(teacher_output[1]) == tuple:
            ...
            student_attn = student_output[2][0].mean(axis = 1)[:, 0, 1:].detach()
            student_attn = student_attn / student_attn.sum(axis = -1, keepdims = True)
            teacher_attn = teacher_output[2][0].mean(axis = 1)[:, 0, 1:].detach()
